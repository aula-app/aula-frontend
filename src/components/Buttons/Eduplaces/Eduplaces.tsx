...
/** Brand button variants, per the Eduplaces partner guidelines. */
export type EduplacesVariant = 'purple-outline' | 'purple' | 'space' | 'mono-outline' | 'mono';

interface EduplacesProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  /** Brand color variant. Only Eduplaces brand styles are permitted. */
  variant?: EduplacesVariant;
  /** Visible label. Must keep the "Eduplaces" brand name. */
  label?: string;
  testId?: string;
}

const EduplacesLogo = () => (
  <svg className="bi" viewBox="0 0 115.28 81" aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg">
...
/**
 * @see https://developer.eduplaces.de/guides/login-button
 * @component Eduplaces
 */
const Eduplaces = forwardRef<HTMLButtonElement, EduplacesProps>(
  (
    {
      variant = 'purple',
      label = 'Login with Eduplaces',
      testId = 'eduplaces-login',
      type = 'button',
      className = '',
      ...restOfProps
    },
    ...
    <button
      ...
      type={type}
      data-testid={testId}
      className={`edu-login-btn edu-login-${variant} ${className}`.trim()}
      {...restOfProps}
    >
      <EduplacesLogo />
      {label}
...
Eduplaces.displayName = 'Eduplaces';
