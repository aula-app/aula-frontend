...
const AddBoxButton = forwardRef<AddBoxRefProps, Props>(({ ideas = [], disabled = false, ...restOfProps }, ref) => {
  ...
  const setIdeasBoxes = async () => {
    const add = ideas.map((user_id) => updates.add.map((box_id) => addIdeaBox(user_id, box_id)));
    const remove = ideas.map((user_id) => updates.remove.map((box_id) => removeIdeaBox(user_id, box_id)));
