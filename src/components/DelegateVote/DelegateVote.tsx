...
const DelegateVote = ({ open, delegate, onClose, triggerRef }: Props) => {
  ...
  return (
    ...
    >
      <Stack minHeight="21.875rem" height={{ xs: 'auto', sm: '21.875rem' }} position="relative" overflow="hidden">
        <Slide direction="right" in={!selected && !delegate} mountOnEnter unmountOnExit>
          <Stack position="absolute" height="100%" width="100%">
            <FilledInput
              ...
              endAdornment={
                <AppIconButton icon="close" title={t('tooltips.close')} size="small" onClick={() => setFilter('')} />
              }
