...
import { BaseTest, describeWithSetup } from '../../shared/base-test';
...
describeWithSetup('CSV Import', () => {
  ...
  test.beforeAll(async () => {
    ...
    roomResponse = await apiClient.addRoom({ room_name: room.name, description_public: room.description, description_internal: room.description });
  ...
  test('Admins should be able to upload CSV file', async () => {
    ...
    await formInteractions.selectAutocompleteOption(adminPage, 'user-room-select', room.name)
  ...
  test('imported users should be added to the destination room', async () => {
    ...
    csvUsers.forEach(async (u) =>
      await expect(UserSelector.locator('div div span'))
        .toHaveText(u.displayName)
    );
  });
